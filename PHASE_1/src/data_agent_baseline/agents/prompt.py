...
3. The task is complete only when you call the `answer` tool. Only call it after you have computed the result from actual data. Set `evidence_step` to a successful observation containing the submitted values. Harmless differences in numeric representation, row order, column aliases, and supported projections are allowed; failed or missing evidence is not.
