...
- `task_id`
- `question`
- `difficulty`（部分 demo release 中可选）
...
如果任务包含视频文件，它会出现在该任务的 `context/` 目录下，例如 `context/video/briefing.mp4`。baseline 会把 `context/` 下找到的第一个视频文件作为 base64 `video_url` 附加到首次模型请求中。下面示例展示同样的 OpenAI-compatible 请求方式。
...
| 工具 | 作用 | 输入 |
| --- | --- | --- |
| `list_context` | 列出 `context/` 下的文件和目录，包括任务中存在的视频文件。 | `max_depth` |
