...
# DataAgent-Bench Phase 2 Starter Kit
...
- `task_id`
- `question`
- `difficulty` (optional in some demo releases)
...
## Minimal Video Input Example

If a task includes a video file, it will appear under the task `context/` directory, for example `context/video/briefing.mp4`. The baseline attaches the first video file it finds under `context/` to the initial model request as a base64 `video_url`. The following example shows the same OpenAI-compatible request pattern directly.
...
video_path = Path("/input/task_1/context/video/briefing.mp4")
    ...
    messages=[
        {
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": "Answer the task question using the provided context video.",
                },
            ],
        }
    ],
...
| Tool | Purpose | Inputs |
| --- | --- | --- |
| `list_context` | List files and directories under `context/`, including any video files present in the task. | `max_depth` |
